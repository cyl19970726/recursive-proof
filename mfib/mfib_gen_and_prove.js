...
const input2 = require("./mfib.input_2.json")
...
async function generateAndVerifyPilStark(inputs) {
    ...
    const executionResult = await mFibExecutor.execute(cmPols.mFibonacci, inputs); 
...
async  function Aggregate_2fib_proofs() {
  // generate proof 1 and proof 2 
  let stark_proof_1 = await generateAndVerifyPilStark(input);
  let stark_proof_2 = await generateAndVerifyPilStark(input2);

  // aggregate proof 1 and proof 2 

}
generateAndVerifyPilStark(input);